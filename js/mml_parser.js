...
MathMLParser.prototype.parse = function (input) {
  this.i = 0;
...
MathMLParser.prototype.expression = function() {
...
MathMLParser.prototype.term = function() {
  var tokens = this.tokens;
  var mrow = $('<mrow>').attr('id', '_' + id);
  id++;
  var mo, mi;
...
MathMLParser.prototype.factor = function() {
  var tokens = this.tokens;
  var token = tokens[this.i++];
  var sign = '';
  var mi, mn, mo;
    ...
    if (tokens[this.i++] === '^') {
      if (sign) {
        ...
        if (isAlpha(token)) {
          ...
          msup.append($('<mn>').text(token).attr('id', '_' + id));
          id++;
        }
        token = tokens[this.i++];
        if (token === '(') {
      ...
      if (isAlpha(token)) {
        ...
        result = $('<mn>').text(sign + token).attr('id', '_' + id);
      }
      id++;
      return result;
    }
